	...
	$output.='<tr><th>Std#</th><th>Name</th> <th>Batch</th><th>Course</th><th>Present</th><th>Absent</th></tr>';
		...
		$qrun = mysqli_query($conn, $sql);
			...
			while($r=mysqli_fetch_assoc($qrun)){
				...
				$output.='<tr> <td>'.$r['student_id'].'</td> <td> '.$r['student_fname'].' '.$r['student_lname'].'</td> <td>'.$r['student_section'].'</td> '.$r['student_class'].'<td> '.$r['student_section'].'</td> <td>'.$present['student_id'].'</td><td>'.$absent['student_id'].'</td></tr>';
			 } 
